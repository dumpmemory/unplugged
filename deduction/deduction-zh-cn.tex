...
range'\ a\ b = \oplus\ z \mapsto
  \begin{cases}
  a > b: & z \\
  \text{否则}: & a\ \oplus\ (range'\ (a+1)\ b\ \oplus\ c) \\
  \end{cases}
\]
...
为了方便使用融合律，我们可以把常见的列表生成操作写为$build...foldr$的形式。这样当用叠加操作和这种形式的操作组合起来时，$\pmb{foldr}...(\pmb{build}...foldr)$，就可以使用融合律化简。
...
为此我们定义由加号分割的若干子算式$t_1 + t_2 + ... + t_n$为$expr = [t_1, t_2, ..., t_n]$：
